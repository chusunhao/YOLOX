...
class YOLOV5ToCOCO(object):
    def __init__(self, dir_path):
        ...
        # 构建COCO格式目录
        self.dst = Path(self.src) / "SPARK2022_COCO_format"
    ...
    def gen_dataset(self, img_paths, target_img_path, target_json, mode):
        ...
        for img_id, img_path in enumerate(tqdm(img_paths, desc=mode), 1):
            ...
            label_path = str(self.src_data
                             / 'labels' / mode / f'{img_path.stem}.txt')
    ...
    def _get_annotation(vertex_info, height, width):
        ...
        cx = (cx * width)
        cy = cy * height
        box_w = (w * width).__round__()
        box_h = (h * height).__round__()

        # left top
        x0 = max(cx - box_w / 2, 0).__round__()
        y0 = max(cy - box_h / 2, 0).__round__()

        # right bottomt
        x1 = min(x0 + box_w, width).__round__()
        y1 = min(y0 + box_h, height).__round__()
...
if __name__ == "__main__":
    dir_path = 'datasets/SPARK2022_YOLOv5'

    converter = YOLOV5ToCOCO(dir_path)
    converter.generate()
