...
def get_images_info_and_annotations(opt):
    ...
    if path.is_dir():
        ...
        file_paths_sample = sample(file_paths, 200)
    else:
    ...
    for file_path in file_paths_sample:
        ...
        if opt.yolo_subdir:
            annotations_path = file_path.parents[2] / "labels" / "val" / label_file_name
            new_file_path = Path("./output/val") / file_path.name
...
if __name__ == "__main__":
    options = get_args()
    options.path = "/home/sstc/PycharmProjects/tph-yolov5/spark_dataset/images/val"
    options.output = "val.json"
    options.yolo_subdir = True
    # options.debug = True
    main(options)
